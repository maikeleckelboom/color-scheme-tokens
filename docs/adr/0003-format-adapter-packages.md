...
roles. DTCG is the motivating example: it can be imported as source material, exported from compiled sets, and converted
...
such as `@color-scheme-tokens/format-dtcg`.

Core must not import format adapters. Format adapters may import `color-scheme-tokens` contracts and helpers, but core
...
Outputs that claim to be `TokenGraphInput`, `TokenLayerInput`, or `CompiledTokenSet` must validate through the matching
...
`@color-scheme-tokens/format-dtcg` is planned as the DTCG format adapter. It is not part of `color-scheme-tokens` 0.1.0.

The first DTCG import surface is expected to be `dtcgSource(input)`, returning source material that can enter
`buildTokenSet()`. The first export surface is expected to be `exportDtcgDocuments(compiled)`, taking a core
`CompiledTokenSet` and producing DTCG documents.
